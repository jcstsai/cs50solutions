...
void hide_banner();
bool is_board_valid();
bool is_temp_board_valid(int[9][9]);
bool load_board();
...
main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        switch (ch)
        {
            ...
            case '8':
            case '9':
                if (g.originalBoard[g.y][g.x] == 0 && !isWon) {
                ...
                if (is_board_valid(g.board) && !isWon) {
                    ...
                    show_banner("Error: You entered an invalid number.");
                    show_cursor();
                }
...
/*
 * bool is_board_valid
 * Returns true if the board is valid else false
 */
bool is_board_valid(int board[9][9]) {
    bool numbers[9];
    ...
    for (int i = 0; i < 9; i++) {
        ...
        for (int j = 0; j < 9; j++){
            if (board[j][i] != 0 && numbers[board[j][i] - 1]) {
                numbers[board[j][i] - 1] = false;
            } else if (board[j][i] != 0) {
                return false;
            }
        }
    ...
    for (int i = 0; i < 9; i += 3) {
        for (int j = 0; j < 9; j += 3) {
            ...
            for (int h = 0; h < 3; h++) {
                for (int k = 0; k < 3; k++) {
                    if (board[i+h][j+k] != 0 && numbers[board[i+h][j+k]-1]) {
                        numbers[board[i+h][j+k]-1] = false;
                    } else if (board[i+h][j+k] != 0) {
                        return false;
                    }
                }
            }
...
load_board()
{
    ...
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            g.originalBoard[i][j] = g.board[i][j];
        }
    }
